...
- **Vitest (web)**: `apps/web/vitest.config.ts` uses jsdom environment with `esbuild: { jsx: 'automatic' }` for React JSX transform. Path aliases `@/` and `@vetea/shared` resolve to source files. The setup file at `__tests__/setup.ts` imports `@testing-library/jest-dom/vitest` matchers and runs `cleanup()` after each test (required because `globals: false`).
- **Playwright E2E**: Installed but browser binaries are NOT downloaded by default. Run `pnpm --filter web exec playwright install chromium` before executing `pnpm --filter web test:e2e`. E2E tests expect a dev server on port 3000 (the Playwright config starts one automatically).